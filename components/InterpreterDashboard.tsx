...
const InterpreterDashboard: React.FC<InterpreterDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'live' | 'history'>(initialTab);
  ...
  const isUsingCustomImage = personaImageUrl !== "https://images.unsplash.com/photo-1559839734-2b71f15360ee?auto=format&fit=crop&q=80&w=1000";
  ...
  const handleDownloadTranscript = (session: SavedSession) => {
    const transcriptText = session.transcriptions.map(t => {
      const role = t.role === 'doctor' ? 'DOCTOR' : 'PATIENT';
      return `[${new Date(t.timestamp).toLocaleTimeString()}] ${role}:\nOriginal: ${t.sourceText}\nTranslated: ${t.translatedText}\n`;
    }).join('\n---\n\n');
    ...
    a.href = url;
    a.download = `Transcript_${session.patientName}_${new Date(session.timestamp).toISOString().split('T')[0]}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const handleDownloadRecording = (session: SavedSession) => {
    if (!session.audioBlob) return;
    const url = URL.createObjectURL(session.audioBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `SessionRecording_${session.patientName}_${new Date(session.timestamp).toISOString().split('T')[0]}.webm`;
    a.click();
    URL.revokeObjectURL(url);
  };
        ...
        <div className="animate-in fade-in duration-300">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
              ...
              <p className="text-slate-500 font-medium text-sm mt-1 text-blue-600 uppercase tracking-widest font-black text-[10px]">Direct Neural Interpretation Link Active</p>
            </div>
            <button 
              onClick={onCloseSession}
              className="px-8 py-3 bg-slate-950 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-rose-600 transition-all shadow-xl active:scale-95"
            >
              End Consultation
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7 bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-[700px] relative">
                ...
                <div className="p-8 border-t border-slate-100 bg-white flex items-center justify-center">
                  ...
                  >
                    {status === SessionStatus.PAUSED ? (
                        ...
                        <><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg> Hold Interaction</>
                    )}
                  </button>
                </div>
              )}
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-slate-950 rounded-[3rem] shadow-2xl border border-slate-800 h-[550px] relative overflow-hidden flex flex-col items-center justify-end group">
                  ...
                  <div className="persona-breathing w-full h-full relative">
                      <div className="persona-blinking w-full h-full">
                          {isUsingCustomImage ? (
                            ...
                            <video 
                              ref={personaVideoRef}
                              // A professional looking medical persona video
                ...
                <div className="relative z-20 w-full p-10 text-center bg-gradient-to-t from-slate-950 to-transparent">
                  <div className={`min-h-[140px] flex items-center justify-center transition-all duration-500 ${isAgentSpeaking ? 'opacity-100 translate-y-0 scale-100' : 'opacity-20 translate-y-4 scale-95'}`}>
                    <p className="text-2xl font-black text-white leading-tight italic drop-shadow-[0_4px_30px_rgba(0,0,0,1)] px-4">
                  ...
                  <div className="mt-8 flex items-center justify-between border-t border-white/10 pt-4">
                    ...
                    <span className="text-[11px] font-black text-blue-400 uppercase tracking-widest italic tracking-wider">V-INTERPRET LIVE</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-xl">
                 <div className="flex items-center justify-between mb-4">
                    <div>
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Interpreter Profile</h4>
                        <p className="text-sm font-bold text-slate-900 mt-1">Real-time Jaw-Sync active</p>
                    </div>
                 </div>
                 ...
                 <div className="flex flex-col gap-3">
                    <button 
                      ...
                      className="w-full py-4 bg-slate-100 text-slate-700 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-slate-200 transition-all flex items-center justify-center gap-4 active:scale-95"
                    >
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-4">
                  <button 
                    onClick={() => handleDownloadTranscript(selectedSession)}
                    className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2"
                  >
                  ...
                  <button 
                    ...
                    className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 flex items-center gap-2"
                  >
                    ...
                    {selectedSessionAudioUrl && (
                        <div className="bg-blue-50/70 p-6 rounded-[2.5rem] border-2 border-blue-100 min-w-[360px] shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <p className="text-[11px] font-black uppercase text-blue-700 tracking-[0.15em]">Clinical Session Log</p>
                            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {savedSessions.length === 0 ? (
                <div className="col-span-full py-48 text-center bg-white/50 rounded-[4rem] border-4 border-dashed border-slate-200">
                    ...
                    <button onClick={onGoHome} className="mt-8 px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl">Back to Home</button>
                </div>
              ) : (
                savedSessions.map(session => (
                  <div key={session.id} onClick={() => setSelectedSession(session)} className="bg-white p-10 rounded-[3rem] border-2 border-slate-50 shadow-sm cursor-pointer hover:border-blue-400 hover:shadow-2xl transition-all group active:scale-[0.98] flex flex-col h-full relative overflow-hidden">
                    ...
                    <p className="text-sm text-slate-500 mb-10 flex-grow font-medium leading-relaxed">{session.transcriptions.length} Interaction cycles recorded.</p>
